    ...
    public partial class Site : System.Web.UI.MasterPage
    {
        ...
        private void SetUserAccess()
        {
            if (Request.QueryString["mid"] != null)
            {
                ...
                menuId = Convert.ToInt32(GeneralFunctions.DecryptQueryString(Request.QueryString["mid"]));
        ...
        private string GenerateList(DataTable MenuTable, StringBuilder StringMenu, DataRow[] PRows)
        {
            foreach (DataRow pRow in PRows)
            {
                ...
                {
                    if (Convert.ToBoolean(pRow["CanView"]))
                    {
                        ...
                        switch (pRow["Name"].ToString())
                        {
                            default:
                                if (pRow["Navigation"].ToString().IndexOf('?') >= 0)
                                    StringMenu.Append("<a href='" + Page.ResolveClientUrl(pRow["Navigation"].ToString()) + "&mid=" + GeneralFunctions.EncryptQueryString(pRow["MenuID"].ToString()) + "'>" + pRow["Name"].ToString() + "</a>");
                                else
                                    StringMenu.Append("<a href='" + Page.ResolveClientUrl(pRow["Navigation"].ToString()) + "?mid=" + GeneralFunctions.EncryptQueryString(pRow["MenuID"].ToString()) + "'>" + pRow["Name"].ToString() + "</a>");
                                break;
                        }
